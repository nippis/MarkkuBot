...
docker exec $CONTAINER sh -c 'mongoexport --db '$DB_NAME' --collection '$CHATS_COLL_NAME'' > $CHATS_FILE
docker exec $CONTAINER sh -c 'mongoexport --db '$DB_NAME' --collection '$WORDS_COLL_NAME'' > $WORDS_FILE
docker exec $CONTAINER sh -c 'mongoexport --db '$DB_NAME' --collection '$BLACKLIST_COLL_NAME'' > $BLACKLIST_FILE
